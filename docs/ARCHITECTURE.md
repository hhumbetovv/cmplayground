...
| Is it "is this section expanded", "which tab is open"? | `FieldState` |
| Is it a text field's in-flight value? | `TextInputState` locally, committed to the holder |

`FieldState` (`core/state/FieldState.kt`) gives each field its own `MutableState`, so writing one
invalidates only its readers, and writing at all requires the `FieldOwner` receiver — the same
"only the owner mutates" guarantee a reducer gives, without the reducer. `TaskEditorUiState` in
`TaskEditorSheet.kt` is the worked example: the note section's expansion never reaches the reducer.

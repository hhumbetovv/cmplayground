...
/**
 * A `State` field even though there is only one: the row must not recompose when the selection
 * changes, only the two chips that flipped.
 */
@Stable
...
/**
 * The row reads nothing, so it composes once. Each chip derives its own `Boolean` from the
 * selection, so toggling a filter recomposes exactly two chips.
 *
